...
class SessionData extends DataClass implements Insertable<SessionData> {
  final int id;
  final int? userId;
  const SessionData({required this.id, this.userId});
  @override
  Map<String, Expression> toColumns(bool nullToAbsent) {
    final map = <String, Expression>{};
    map['id'] = Variable<int>(id);
  ...
  SessionData copyWith({int? id, Value<int?> userId = const Value.absent()}) =>
      SessionData(
        id: id ?? this.id,
...
class SessionCompanion extends UpdateCompanion<SessionData> {
  ...
  SessionCompanion copyWith({Value<int>? id, Value<int?>? userId}) {
    return SessionCompanion(id: id ?? this.id, userId: userId ?? this.userId);
  }
...
final class $$UsersTableReferences
  ...
  _sessionRefsTable(_$AppDatabase db) => MultiTypedResultKey.fromTable(
    db.session,
    aliasName: $_aliasNameGenerator(db.users.id, db.session.userId),
  );
...
final class $$SessionTableReferences
  ...
  static $UsersTable _userIdTable(_$AppDatabase db) => db.users.createAlias(
    $_aliasNameGenerator(db.session.userId, db.users.id),
  );
        ...
        TableManagerState(
          ...
          updateCompanionCallback:
              ({
                Value<int> id = const Value.absent(),
                Value<int?> userId = const Value.absent(),
              }) => SessionCompanion(id: id, userId: userId),
          createCompanionCallback:
              ({
                Value<int> id = const Value.absent(),
                Value<int?> userId = const Value.absent(),
              }) => SessionCompanion.insert(id: id, userId: userId),
          withReferenceMapper: (p0) => p0
              .map(
                (e) => (
                  e.readTable(table),
